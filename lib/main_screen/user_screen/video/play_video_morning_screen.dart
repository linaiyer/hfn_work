...
class play_video_morning_screen extends StatefulWidget {
  final String title;
  final String url;
  final int week;
  final int day;

  const play_video_morning_screen({
    Key? key,
    required this.title,
    required this.url,
    required this.week,
  ...
  @override
  _play_video_morning_screen createState() => _play_video_morning_screen();
}

class _play_video_morning_screen extends State<play_video_morning_screen> {
  ...
  Future<void> _recordSession(DateTime start, DateTime end) async {
    ...
    // increment morningSec instead of generic durationSec
    await docRef.update({
      'morningSec': FieldValue.increment(listened),
    });
  }
        ...
        body: _loading
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Start your day with a heart-centered attitude. Cultivating a balanced and calm state of mind through a simple meditation practice allows us to embrace the highs and lows of life with the right understanding and humility.  Please click the above link to experience it! For the best results, we suggest you be consistent with your practice.',
                style: TextStyle(
                  fontFamily: 'WorkSans',
                  fontSize: 17,
                  color: Color(0xFF333333),
